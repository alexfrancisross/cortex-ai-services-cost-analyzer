...
SELECT 'CORTEX_AISQL_USAGE_HISTORY' AS view_name, COUNT(*) AS row_count, MAX(start_time) AS latest_row
...
SELECT 'CORTEX_CODE_CLI_USAGE_HISTORY' AS view_name, COUNT(*) AS row_count, MAX(start_time) AS latest_row
...
(
    SELECT
      ...
      + COALESCE((SELECT SUM(token_credits) FROM SNOWFLAKE.ACCOUNT_USAGE.CORTEX_AISQL_USAGE_HISTORY           WHERE start_time >= DATEADD('day', -30, CURRENT_TIMESTAMP())), 0)
