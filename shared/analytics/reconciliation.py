...
class ReconciliationEngine:
    ...
    def validate_reconciliation_accuracy(self, start_date, end_date, 
                                       target_accuracy: float = 99.5) -> Dict[str, Any]:
        """
        Validate that reconciliation meets target accuracy requirements.
        ...
        try:
            reconciliation = self.perform_three_tier_reconciliation(start_date, end_date)
            
            # Test 1: Primary reconciliation variance
            primary_variance = reconciliation.get('variance_hourly_granular_abs', float('inf'))
            accuracy = 100 - primary_variance
            
            validation_results['tests']['primary_accuracy'] = {
                'test': 'Hourly vs Granular Accuracy',
                'result': accuracy,
                'target': target_accuracy,
                'passed': accuracy >= target_accuracy,
                'variance_pct': primary_variance
            }
            
            # Test 2: Service coverage
            coverage = reconciliation.get('service_coverage', 0)
            validation_results['tests']['service_coverage'] = {
                'test': 'Service Coverage',
                'result': coverage,
                'target': 95.0,  # Minimum 95% coverage
                'passed': coverage >= 95.0
            }
            
            # Test 3: Data completeness
            accessible_services = reconciliation.get('granular_services', {}).get('accessible_services', 0)
            validation_results['tests']['data_completeness'] = {
                'test': 'Service Table Access',
                'result': accessible_services,
                'target': 4,  # Minimum 4 out of 6 services
                'passed': accessible_services >= 4
            }
            
            # Overall validation status
            all_tests_passed = all(test['passed'] for test in validation_results['tests'].values())
            validation_results['validation_passed'] = all_tests_passed
            
            # Generate recommendations
            if not all_tests_passed:
                validation_results['recommendations'] = []
                
                if not validation_results['tests']['primary_accuracy']['passed']:
                    validation_results['recommendations'].append(
                        "Investigate high variance between hourly and granular services"
                    )
                
                if not validation_results['tests']['service_coverage']['passed']:
                    validation_results['recommendations'].append(
                        "Check for missing or new Cortex service types"
                    )
                
                if not validation_results['tests']['data_completeness']['passed']:
                    validation_results['recommendations'].append(
                        "Verify access permissions to all Cortex usage tables"
                    )
    ...
    def get_variance_trend(self, days_back: int = 30) -> pd.DataFrame:
        """
        Get variance trend over time to identify patterns.
        
        Args:
            days_back: Number of days to analyze
            
        Returns:
            DataFrame with daily variance calculations
        """
        trend_data = []
        ...
        try:
            ...
            for i in range(days_back):
                ...
                try:
                    results = self.perform_three_tier_reconciliation(day_start, day_end)
                    
                    trend_data.append({
                        'date': day_start,
                        'hourly_credits': results['account_hourly']['credits'],
                        'granular_credits': results['granular_services']['credits'],
                        'variance_pct': results.get('variance_hourly_granular', 0),
                        'service_coverage': results.get('service_coverage', 0),
                        'status': results.get('reconciliation_status', 'UNKNOWN')
                    })
                    
                except Exception:
                    # Skip days with data issues
                    continue
            
            return pd.DataFrame(trend_data).sort_values('date')
            
        except Exception as e:
            st.error(f"Failed to generate variance trend: {str(e)}")
            return pd.DataFrame()
